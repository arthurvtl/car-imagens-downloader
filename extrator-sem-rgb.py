...
Diferente do extrator.py original, este script NÃO converte as imagens de uso do solo para RGB.
Ele salva as imagens de uso do solo como imagens de 1 banda (tons de cinza) onde cada pixel
representa o ID da classe. Os IDs são consistentes entre 2012 e 2020.
...
CLASSES_IDS = {
    ...
    "Área Edificada": 2,
    "Brejo": 3,
    "Campo Rupestre/Altitude": 4,
    "Cultivo Agrícola - Abacaxi": 5,
    "Cultivo Agrícola - Banana": 6,
    "Cultivo Agrícola - Café": 7,
    "Cultivo Agrícola - Cana-de-Açúcar": 8,
    "Cultivo Agrícola - Coco-da-Baía": 9,
    "Cultivo Agrícola - Mamão": 10,
    "Outros Cultivos Permanentes": 11,
    "Outros Cultivos Temporários": 12,
    "Extração Mineração": 13,
    "Macega": 14,
    "Mangue": 15,
    "Massa D'Água": 16,
    "Mata Nativa": 17,
    "Mata em Regeneração": 18,
    "Outros": 19,
    "Pastagem": 20,
    "Eucalipto": 21,
    "Pinus": 22,
    ...
    "Mata Nativa em Estágio Inicial de Regeneração": 18,
    "Cultivo Agrícola - Outros Cultivos Permanentes": 11,
    "Reflorestamento - Eucalipto": 21,
    "Cultivo Agrícola - Coco-Da-Baía": 9,
    "Cultivo Agrícola - Cana-De-Açúcar": 8,
    "Reflorestamento - Seringueira": 23,
    "Reflorestamento - Pinus": 22,
    "Cultivo Agrícola - Outros Cultivos Temporários": 12,
}
...
def converter_rgb_para_id(caminho_imagem: Path) -> None:
    """Abre uma imagem RGB e a converte para 1 banda com IDs das classes."""
    ...
    try:
        ...
        altura, largura = dados.shape[1], dados.shape[2]
        ids_array = np.zeros((altura, largura), dtype=np.uint8)
        ...
        with rasterio.open(caminho_imagem, 'w', **perfil) as dst:
            dst.write(ids_array, 1)
            
    except Exception as e:
        logger.error(f"Erro ao converter RGB para ID na imagem {caminho_imagem}: {e}")
...
async def processar_amostra_async(
    ...
    async with semaforo:
        ...
        pasta_satelite = pasta / "imagens satelites"
        pasta_uso_solo = pasta / "imagens segmentadas"
        pasta_satelite.mkdir(parents=True, exist_ok=True)
        pasta_uso_solo.mkdir(parents=True, exist_ok=True)
        caminho_satelite = pasta_satelite / f"amostra_{numero_amostra}.tif"
        caminho_uso_solo = pasta_uso_solo / f"amostra_{numero_amostra}.tif"
        ...
        if status_uso_solo == "ok":
            ...
            await loop.run_in_executor(None, converter_rgb_para_id, caminho_uso_solo)
...
def processar_ano_2012(
    ...
    id_para_id_novo: dict[int, int] = {0: 0}
    for _, linha in gdf_uso_solo[[col_id, col_classe]].drop_duplicates().iterrows():
        ...
        id_novo = CLASSES_IDS.get(nome_classe)
        if id_novo is not None:
            id_para_id_novo[classe_id] = id_novo
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            pasta_satelite = pasta_saida_path / "imagens satelites"
            pasta_uso_solo = pasta_saida_path / "imagens segmentadas"
            pasta_satelite.mkdir(parents=True, exist_ok=True)
            pasta_uso_solo.mkdir(parents=True, exist_ok=True)
            caminho_satelite = pasta_satelite / f"amostra_{numero_amostra}.tif"
            caminho_uso_solo = pasta_uso_solo / f"amostra_{numero_amostra}.tif"
            ...
            for geom, classe_id_val in zip(
                ...
                try:
                    ...
                    continue
                id_novo = id_para_id_novo.get(cid)
                if id_novo is not None:
