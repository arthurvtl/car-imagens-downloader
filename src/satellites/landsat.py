"""
src.satellites.landsat
Provedor Landsat 8/9 via STAC (Microsoft Planetary Computer).

Bandas principais:
  red (30m), green (30m), blue (30m),
  nir08 (30m), swir16 (30m), swir22 (30m),
  lwir11 (TIR, 100m) — IMPORTANTE para análise térmica
"""
...
class LandsatProvider(BaseSatellite):
    """Provedor Landsat 8/9 Collection 2 Level-2 via Planetary Computer STAC."""
    ...
    async def download(
        ...
        Landsat usa escalamento linear de reflectância para uint8.
        """
    ...
    def validate(self) -> bool:
        if not self.sat_config.catalog_url:
            self.logger.error("Landsat: catalog_url não configurado.")
            return False
        if not self.sat_config.collection:
            self.logger.error("Landsat: collection não configurada.")
            return False
